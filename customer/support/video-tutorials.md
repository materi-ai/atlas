...
```mermaid
flowchart TD
    A[Choose Your Path] --> B{What's your role?}

    B -->|Individual User| C[Essentials Path]
    B -->|Team Lead| D[Collaboration Path]
    B -->|Administrator| E[Admin Path]
    B -->|Developer| F[API & Integration Path]

    C --> G[Getting Started]
    C --> H[Document Creation]
    C --> I[Formatting Mastery]

    D --> G
    D --> J[Team Collaboration]
    D --> K[Project Management]

    E --> L[Workspace Setup]
    E --> M[User Management]
    E --> N[Security Configuration]

    F --> O[API Basics]
    F --> P[Webhooks & Events]
    F --> Q[Custom Integrations]
```
